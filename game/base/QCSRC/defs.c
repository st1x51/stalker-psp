...
.float		health;
.float		frags;
.float		weapon;			// one of the IT_BERRET, etc flags
.string		weaponmodel;
.float		weaponframe;
.float		currentammo;
...
.float	      renderamt;                          // Render Level (0-255) 

.float	      rendermode;                         // Render Mode

.vector       rendercolor;                        // Render Color (RGB)
...
float   FL_ARCHIVE_OVERRIDE		= 1048576;
// player flags
...
float   SVC_SCREENFADE  = 36;
float   SVC_BSPDECAL  	= 38;
float	SVC_FOG			= 40;
float   SVC_SHOWLMP		= 41;
float   SVC_HIDELMP		= 42;
float   SVC_SHOWSTRING	= 43;
float   SVC_HIDESTRING	= 44;
float	TE_SPIKE		= 0;
...
string (string xmlname,string idname,string texttoreturn)xmlparsetext = #85;
