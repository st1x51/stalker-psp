...
void Sbar_Init (void)
{
	...
	ui_health = Draw_PicFromWad ("ui_health");
	ui_map = Draw_PicFromWad ("ui_map");
	ui_ammo = Draw_PicFromWad ("ui_ammo");
	ui_stamina = Draw_PicFromWad ("ui_stamina");
}
...
/*
================
Sbar_DrawColoredString
color c r[16] g[16] b[16]
================
*/
void Sbar_DrawColoredString (int x, int y, char *str, int red)
{
	if (cl.gametype == GAME_DEATHMATCH)
		Draw_ColoredString (x /*+ ((vid.width - 320)>>1)*/, y+ vid.height-SBAR_HEIGHT, str, red);
	else
		Draw_ColoredString (x + ((vid.width - 320)>>1), y+ vid.height-SBAR_HEIGHT, str, red);
...
void Sbar_Draw (void)
{
   ...
   // armor
	...
	if (vid.width > 320 && cls.state == ca_connected) 
	{
		if (cl.gametype == GAME_DEATHMATCH)
			Sbar_MiniDeathmatchOverlay ();
		
		// map
		Sbar_DrawPic (1, 1, ui_map);
		
		// stamina
		Sbar_DrawPic (1, vid.height - 65, ui_stamina);
		
		// health and armor	
		Sbar_DrawPic (vid.width - 104, vid.height - 90, ui_health);
		
		Draw_Fill (vid.width - 84, vid.height - 65, 67, 8, GU_RGBA(200, 200, 200, 75));
		Draw_Fill (vid.width - 84, vid.height - 65, (67 * (cl.stats[STAT_HEALTH] > 0 ? cl.stats[STAT_HEALTH] : 0) / 100), 8, GU_RGBA(150, 0, 0, 255));
		
		Draw_Fill (vid.width - 84, vid.height - 77, 67, 8, GU_RGBA(200, 200, 200, 75));
		Draw_Fill (vid.width - 84, vid.height - 77, (67 * (cl.stats[STAT_ARMOR] > 0 ? cl.stats[STAT_ARMOR] : 0) / 100), 8, GU_RGBA(0, 35, 255, 255));
		
		// ammo
		char		str_ammo[3];
		
		Sbar_DrawPic (vid.width - 101, vid.height - 45, ui_ammo);
		Draw_Character (vid.width - 37, vid.height - 27,  175);
		
		sprintf (str_ammo," %1i", cl.stats[STAT_SHELLS]);
		M_Print (vid.width - 33, vid.height - 27,  str_ammo);
		sprintf (str_ammo," %3i", cl.stats[STAT_AMMO]);
		M_Print (vid.width - 73, vid.height - 27,  str_ammo);
	}
}
