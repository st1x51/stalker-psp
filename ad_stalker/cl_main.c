...
// rotate binary objects locally
		...
		if (ent->effects & EF_MUZZLEFLASH)
		{
			...
			if (i == cl.viewentity && qmb_initialized && r_part_muzzleflash.value)
			{
				...
				smokeorg[2] += 24;	// move it up

				// adjust for current view angles
				AngleVectors (cl.viewangles, fv, rv, uv);

				// forward vector
				VectorMA (smokeorg, 30, fv, smokeorg);

			if (cl.stats[STAT_ACTIVEWEAPON] == 1 || cl.stats[STAT_ACTIVEWEAPON] == 4)
			{
...
#endif
		if (!strcmp(ent->model->name, "sprites/fire.spr32"))
		{
			if (qmb_initialized && r_part_flames.value)
			{
					QMB_FlyParticles(ent->origin,20); 
			}
		}
