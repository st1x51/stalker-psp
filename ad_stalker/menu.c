...
void M_Main_Draw (void)
{
	...
	M_DrawTransPic (395, 120 + (m_main_cursor *20), Draw_CachePic( va("gfx/menu/ui_ani_cursor_0%i.lmp", f+1 ) ));
}
...
void M_SinglePlayer_Draw (void)
{
	...
	//Draw_Pic (vid.width - 180, vid.height - 170 + (m_singleplayer_cursor * 18), p);
}
...
void M_Load_Draw (void)
{
	...
	for (i=0 ; i< MAX_SAVEGAMES; i++)
		M_PrintWhite (20, 32 + 8*i, m_filenames[i]);

// line cursor
	M_DrawCharacter (10, 32 + load_cursor*8, 12+((int)(realtime*4)&1));
}
...
void M_Maps_Draw (void)
{
	...
	{
		M_PrintWhite (20, 32 + 8*(i), maps_list[i+maps_stage]);
    }
    
// line cursor
	M_DrawCharacter (10, 32 + maps_cursor * 8, 12+((int)(realtime*4)&1));
}
...
#ifdef PSP
	M_PrintWhite (20, 97, "Infrastructure ");
	M_DrawCheckbox (220, 97, tcpipAvailable );

#if 0
	// Sort out adhoc later
	M_PrintWhite (20, 117,  "Adhoc          ");
...
void M_Setup_Draw (void)
{
	...
	M_PrintWhite (20, 40, "Access Point");
	M_DrawTextBox (160, 32, 16, 1);
	M_PrintWhite (168, 40, setup_accesspoint);
#endif

	M_PrintWhite (20, 40+offset, "Hostname");
	M_DrawTextBox (160, 32+offset, 16, 1);
	M_PrintWhite (168, 56, setup_hostname);

	M_PrintWhite (20, 56+offset, "Your name");
	M_DrawTextBox (160, 48+offset, 16, 1);
	M_PrintWhite (168, 56+offset, setup_myname);

	M_PrintWhite (20, 80+offset, "Shirt color");
	M_PrintWhite (20, 104+offset, "Pants color");

	M_DrawTextBox (20, 140+offset-8, 14, 1);
	M_PrintWhite (72, 140+offset, "Accept Changes");

	p = Draw_CachePic ("gfx/bigbox.lmp");
	M_DrawTransPic (160, 20+offset, p);
...
void M_Net_Draw (void)
{
	...
	f += 8;
	M_PrintWhite (f, 142, net_helpMessage[m_net_cursor*4+0]);
	M_PrintWhite (f, 150, net_helpMessage[m_net_cursor*4+1]);
	M_PrintWhite (f, 158, net_helpMessage[m_net_cursor*4+2]);
	M_PrintWhite (f, 166, net_helpMessage[m_net_cursor*4+3]);
...
void M_AdjustSliders (int dir)
{
    ...
    else if (m_submenu == 1)
    {
       	switch (options_cursor)
        {
			...
			case OPT_YAW_VIEW:
				cl_yawspeed.value += dir*5;
				if (cl_yawspeed.value < 10)
					cl_yawspeed.value = 10;
				if (cl_yawspeed.value > 400)
					cl_yawspeed.value = 400;
				Cvar_SetValue ("cl_yawspeed", cl_yawspeed.value);
				break;
				
			case OPT_PITCH_VIEW:
				cl_pitchspeed.value += dir*5;
				if (cl_pitchspeed.value < 10)
					cl_pitchspeed.value = 10;
				if (cl_pitchspeed.value > 400)
					cl_pitchspeed.value = 400;
				Cvar_SetValue ("cl_pitchspeed", cl_pitchspeed.value);
				break;
...
void M_Options_Draw (void)
{
	...
	M_PrintWhite (20, 32+(OPT_CUSTOMIZE*8), "Customize controls");
	M_PrintWhite (20, 32+(OPT_CONSOLE*8),   "Go to console");
	M_PrintWhite (20, 32+(OPT_DEFAULTS*8),  "Reset to defaults");

	switch (m_submenu) {
        case 0:    
			M_PrintWhite (20, 32+(OPT_SCRSIZE*8),   "Screen size");
			...
			M_PrintWhite (20, 32+(OPT_SNDVOL*8),   "Sound Volume");
			r = volume.value;
			M_DrawSlider (174, 32+(OPT_SNDVOL*8), r);

			M_PrintWhite (20, 32+(OPT_MUSICTYPE*8),"Music Type");
			if (strcmpi(bgmtype.string,"cd") == 0)
				M_PrintWhite (174, 32+(OPT_MUSICTYPE*8), "CD/MP3");
			else
				M_PrintWhite (174, 32+(OPT_MUSICTYPE*8), "None");
		...
		#ifdef PSP_HARDWARE_VIDEO
			M_PrintWhite (20, 32+(OPT_WATERTRANS*8), "Water tranparency");
			M_DrawSlider (174, 32+(OPT_WATERTRANS*8), r_wateralpha.value);
		
			M_PrintWhite (20, 32+(OPT_TEX_SCALEDOWN*8), "Texture Scale Down");
			M_DrawCheckbox (174, 32+(OPT_TEX_SCALEDOWN*8), r_tex_scale_down.value);
		
			M_PrintWhite (20, 32+(OPT_SIMPLE_PART*8),    "Simple Particles");
			M_DrawCheckbox (174, 32+(OPT_SIMPLE_PART*8), r_particles_simple.value);
			
			M_PrintWhite (20, 32+(OPT_DITHERING*8),      "Dithering");
			M_DrawCheckbox (174, 32+(OPT_DITHERING*8), r_dithering.value);
			
		#endif
            break;
        case 1:
        	M_PrintWhite (20, 32+(OPT_ALWAYRUN*8),        "Always Run");
			M_DrawCheckbox (174, 32+(OPT_ALWAYRUN*8), cl_forwardspeed.value > 200);
		    
		#ifdef PSP
			M_PrintWhite (20, 32+(OPT_IN_SPEED*8), 		 "A-Nub Speed");
		#else
			M_PrintWhite (20, 32+(OPT_IN_SPEED*8), 		 "Mouse Speed");
		#endif
			r = (in_sensitivity.value - 1)/10;
			M_DrawSlider (174, 32+(OPT_IN_SPEED*8), r);

			M_PrintWhite (20, 32+(OPT_IN_ACCELERATION*8), "A-Nub Acceleration");
			r = 1.0f -((in_acceleration.value - 0.5f)/1.5f);
			M_DrawSlider (174, 32+(OPT_IN_ACCELERATION*8), r);

			M_PrintWhite (20, 32+(OPT_IN_TOLERANCE*8), 	 "A-Nub Tollerance");
			r = (in_tolerance.value )/1.0f;
			M_DrawSlider (174, 32+(OPT_IN_TOLERANCE*8), r);
		
			M_PrintWhite (20, 32+(OPT_IN_X_ADJUST*8), 	 "Adjust Axis X");	
			r = (128+in_x_axis_adjust.value)/255;
			M_DrawSlider (174, 32+(OPT_IN_X_ADJUST*8), r);

			M_PrintWhite (20, 32+(OPT_IN_Y_ADJUST*8), 	 "Adjust Axis Y");	
			r = (128+in_y_axis_adjust.value)/255;
			M_DrawSlider (174, 32+(OPT_IN_Y_ADJUST*8), r);
			
			// rotate view
			M_PrintWhite (20, 32+(OPT_YAW_VIEW*8), 		 "Viewangles YAW");
			r = cl_yawspeed.value / 400;
			M_DrawSlider (174, 32+(OPT_YAW_VIEW*8), r);
			
			M_PrintWhite (20, 32+(OPT_PITCH_VIEW*8), 		 "Viewangles PITCH");
			r = cl_pitchspeed.value / 400;
			M_DrawSlider (174, 32+(OPT_PITCH_VIEW*8), r);
		
		
		#ifdef PSP
			M_PrintWhite (20, 32+(OPT_INVMOUSE*8),        "Invert A-Nub");
		#else
			M_PrintWhite (20, 32+(OPT_INVMOUSE*8),        "Invert Mouse");
		#endif
			M_DrawCheckbox (174, 32+(OPT_INVMOUSE*8), m_pitch.value < 0);
		
			M_PrintWhite (20, 32+(OPT_MOUSELOOK*8),       "A-Nub Look");
			M_DrawCheckbox (174, 32+(OPT_MOUSELOOK*8), in_analog_freelook.value);

			M_PrintWhite (20, 32+(OPT_NOMOUSE*8),         "Disable A-Nub");
			M_DrawCheckbox (174, 32+(OPT_NOMOUSE*8), in_analog_disable.value );
		
			M_PrintWhite (20, 32+(OPT_MOUSESTAFE*8),		 "A-Nub Stafing");
			M_DrawCheckbox (174, 32+(OPT_MOUSESTAFE*8), in_analog_strafe.value );
			break;
	}
	
	M_PrintWhite (20, 32+(OPT_SUBMENU*8),	 "Select Submenu");
    switch (m_submenu)
        {
        case 0:
            M_PrintWhite (174, 32+(OPT_SUBMENU*8), "More options");         
            break;
        case 1:
            M_PrintWhite (174, 32+(OPT_SUBMENU*8), "Less options");         
            break;
        default:
            break;
        }                         	
	// Cursor
	M_DrawCharacter (10, 32 + options_cursor*8, 12+((int)(realtime*4)&1));
}
...
{"impulse 15", 		"reload weapon"},
{"+jump", 			"jump"},
{"+crouch",		    "crouch"},
{"+use",		    "use"},
{"impulse 16",		"flashlight"},
};
...
#ifdef PSP
	if (bind_grab)
		M_PrintWhite (12, 32, "Press a button for this action");
	else
		M_PrintWhite (18, 32, "Press CROSS to change, SQUARE to clear");
#else
	if (bind_grab)
...
// search for known bindings
	for (i=0 ; i<NUMCOMMANDS ; i++)
	{
		...
		M_PrintWhite (20, y, bindnames[i][1]);
		...
		else
		{
			...
			if (keys[1] != -1)
			{
				...
				M_PrintWhite (140 + x + 32, y, Key_KeynumToString (keys[1]));
			}
		}
	}
	...
	if (bind_grab)
		...
		M_DrawCharacter (10, 48 + keys_cursor*8, 12+((int)(realtime*4)&1));
}
...
void M_Quit_Draw (void)
{
	...
	M_PrintWhite (16, 36,  " John Carmack       Adrian Carmack\n");
	M_PrintWhite (16, 44,  " Michael Abrash     Kevin Cloud\n");
	M_PrintWhite (16, 52,  " John Cash          Paul Steed\n");
	M_PrintWhite (16, 60,  " Dave 'Zoid' Kirsch\n");
	M_PrintWhite (16, 68,  "Design             Biz\n");
	M_PrintWhite (16, 76,  " John Romero        Jay Wilbur\n");
	M_PrintWhite (16, 84,  " Sandy Petersen     Mike Wilson\n");
	M_PrintWhite (16, 92,  " American McGee     Donna Jackson\n");
	M_PrintWhite (16, 100,  " Tim Willits        Todd Hollenshead\n");
	M_PrintWhite (16, 108, "Support            Projects\n");
	M_PrintWhite (16, 116, " Barrett Alexander  Shawn Green\n");
	M_PrintWhite (16, 124, "Sound Effects\n");
	M_PrintWhite (16, 132, " Trent Reznor and Nine Inch Nails\n\n");
	...
	M_PrintWhite (112, 84,	"      Really quit?      ");
	M_PrintWhite (112, 92,	"                        ");
	M_PrintWhite (112, 100,	"  Press CROSS to quit,  ");
	M_PrintWhite (112, 108,	" or CIRCLE to continue. ");
#else
	M_DrawTextBox (56, 76, 24, 4);
	M_PrintWhite (64, 84,  quitMessage[msgNumber*4+0]);
	M_PrintWhite (64, 92,  quitMessage[msgNumber*4+1]);
	M_PrintWhite (64, 100, quitMessage[msgNumber*4+2]);
	M_PrintWhite (64, 108, quitMessage[msgNumber*4+3]);
#endif
}
//=============================================================================
/* OSK IMPLEMENTATION */
...
void M_OSK_Draw (void) // keyboard console
{
#ifdef PSP
	int x,y;
	int i;
...
void M_SerialConfig_Draw (void)
{
	...
	M_PrintWhite (basex, 32, va ("%s - %s", startJoin, directModem));
	basex += 8;

	M_PrintWhite (basex, serialConfig_cursor_table[0], "Port");
	M_DrawTextBox (160, 40, 4, 1);
	M_PrintWhite (168, serialConfig_cursor_table[0], va("COM%u", serialConfig_comport));

	M_PrintWhite (basex, serialConfig_cursor_table[1], "IRQ");
	M_DrawTextBox (160, serialConfig_cursor_table[1]-8, 1, 1);
	M_PrintWhite (168, serialConfig_cursor_table[1], va("%u", serialConfig_irq));

	M_PrintWhite (basex, serialConfig_cursor_table[2], "Baud");
	M_DrawTextBox (160, serialConfig_cursor_table[2]-8, 5, 1);
	M_PrintWhite (168, serialConfig_cursor_table[2], va("%u", serialConfig_baudrate[serialConfig_baud]));

	if (SerialConfig)
	{
		M_PrintWhite (basex, serialConfig_cursor_table[3], "Modem Setup...");
		if (JoiningGame)
		{
			M_PrintWhite (basex, serialConfig_cursor_table[4], "Phone number");
	...
	if (JoiningGame)
	{
		...
		M_PrintWhite (basex+8, serialConfig_cursor_table[5], "Connect");
	}
	else
	{
		M_DrawTextBox (basex, serialConfig_cursor_table[5]-8, 2, 1);
		M_PrintWhite (basex+8, serialConfig_cursor_table[5], "OK");
	}
...
void M_ModemConfig_Draw (void)
{
	...
	if (modemConfig_dialing == 'P')
		M_PrintWhite (basex, modemConfig_cursor_table[0], "Pulse Dialing");
	else
		M_PrintWhite (basex, modemConfig_cursor_table[0], "Touch Tone Dialing");

	M_PrintWhite (basex, modemConfig_cursor_table[1], "Clear");
	M_DrawTextBox (basex, modemConfig_cursor_table[1]+4, 16, 1);
	M_PrintWhite (basex+8, modemConfig_cursor_table[1]+12, modemConfig_clear);
	if (modemConfig_cursor == 1)
		M_DrawCharacter (basex+8 + 8*strlen(modemConfig_clear), modemConfig_cursor_table[1]+12, 10+((int)(realtime*4)&1));

	M_PrintWhite (basex, modemConfig_cursor_table[2], "Init");
	...
	M_PrintWhite (basex, modemConfig_cursor_table[3], "Hangup");
	...
	M_PrintWhite (basex+8, modemConfig_cursor_table[4], "OK");
...
void M_GameOptions_Draw (void)
{
	...
	if (skill.value == 0)
		...
		M_PrintWhite (160, 80, "Hard difficulty");
	else
		M_PrintWhite (160, 80, "Nightmare difficulty");
