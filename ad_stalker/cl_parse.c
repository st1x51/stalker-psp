...
char *svc_strings[] =
{
	...
	"svc_addangle",
	"svc_fog",    // 38		// [byte] start [byte] end [byte] red [byte] green [byte] blue [float] time
...
// [always sent]	if (bits & SU_ITEMS)
	...
	*/
	...
	i = MSG_ReadByte ();
	if (cl.stats[STAT_STAMINA] != i)
	{
		cl.stats[STAT_STAMINA] = i;
		Hud_Changed ();
	}
	i = MSG_ReadByte ();
	if (cl.stats[STAT_STATE] != i)
	{
		cl.stats[STAT_STATE] = i;
		Hud_Changed ();
	}
}
