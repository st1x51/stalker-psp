...
void PF_fabs (void)
{
	...
	v = G_FLOAT(OFS_PARM0);
	#ifdef PSP_VFPU
	G_FLOAT(OFS_RETURN) = vfpu_fabsf(v);
	#else
...
void PF_AddParticle (void)
{
	...
	time = G_FLOAT(OFS_PARM4);
	color = G_FLOAT(OFS_PARM5);
...
ebfs_builtin_t pr_ebfs_builtins[] =
{
	...
	{   5, "addparticle", PF_AddParticle },				// void(entity e, vector min, vector max) setabssize = #5;
