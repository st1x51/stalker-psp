...
void QMB_InitParticles (void)
{
	...
	particle_mode = pm_qmb; //pm_classic
...
void QMB_MuzzleFlash(vec3_t org)
{
	...
	if (!(ISUNDERWATER(TruePointContents(org))))
	{
		AddParticle (p_muzzleflash, org, 1, 10,0.8, NULL, zerodir);
		AddParticle (p_dpsmoke, org, 1, 15, 0.825f, NULL, zerodir);
	}
}
...
void QMB_Q3Gunshot (vec3_t org, int skinnum, float alpha)
{
	...
	ent->skinnum = skinnum;
	ent->rendermode = RENDER_ADDITIVE;
	ent->renderamt = alpha;
