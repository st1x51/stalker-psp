...
void Draw_Init (void)
{
	...
	// it into a texture
	nonetexture = loadtextureimage ("gfx/missingtexture", 0, 0, qfalse, GU_NEAREST); 
	draw_chars = static_cast<byte*>(W_GetLumpName ("conchars"));
	for (i=0 ; i<256*64 ; i++)
		if (draw_chars[i] == 0)
			draw_chars[i] = 255;	// proper transparent color
		
	draw_chars_rus = static_cast<byte*>(W_GetLumpName ("conchars"));
	for (i=0 ; i<256*64 ; i++)
		if (draw_chars_rus[i] == 0)
			draw_chars_rus[i] = 255;	// proper transparent color
	// now turn them into textures
	char_texture = loadtextureimage ("gfx/conchars", 0, 0, qfalse, GU_NEAREST);
    if (char_texture == 0)// did not find a matching TGA...
	{
	   char_texture = GL_LoadTexture ("gfx/conchars", 256, 256, draw_chars, 1, qfalse, GU_NEAREST, 0);
	}
	
	char_texture_rus = loadtextureimage ("gfx/conchars_rus", 0, 0, qfalse, GU_NEAREST);
    if (char_texture_rus == 0)// did not find a matching TGA...
	{
	   char_texture_rus = GL_LoadTexture ("gfx/conchars_rus", 256, 256, draw_chars_rus, 1, qfalse, GU_NEAREST, 0);
	}
	start = Hunk_LowMark();
...
void Draw_String_Rus (int x, int y, char *str)
{
	 /*
	intraFont* arib;   
	arib = intraFontLoad("ltn0.pgf",INTRAFONT_STRING_UTF8);
	intraFontSetStyle(arib,0.5f,0xFFFFFFFF,0xFF7F7F7F,0.f,0);  
	intraFontPrint(arib,(float)x,(float)y,str);	
