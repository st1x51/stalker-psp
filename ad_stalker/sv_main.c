...
//New vars
		...
		if (ent->baseline.bodygroup != ent->v.bodygroup)
			bits |= U_BODYGROUP;
		if (ent->v.health > 0 && ent->v.health <= 100)
			bits |= U_HEALTHVAL;		
...
// [always sent]	if (bits & SU_ITEMS)
	...
	MSG_WriteByte (msg, ent->v.stamina);
	MSG_WriteByte (msg, ent->v.pl_state);
}
...
void SV_CreateBaseline (void)
{
	...
	//
		...
		svent->baseline.sequence = svent->v.sequence; // 0
		svent->baseline.bodygroup = svent->v.bodygroup; // 0
//New vars
